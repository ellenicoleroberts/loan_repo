...
# This function loads a CVS file from the filepath defined in `csvpath.  Gets all of the bank data into Python.
...
# from the file defined in `file_path`
def load_bank_data(file_path):
    csvpath = Path(file_path)   #WHY DO WE MAKE TWO FILE PATHS?
    return load_csv(csvpath)
...
def filter_max_loan_size(loan_amount, bank_list):
    ...
    return loan_size_approval_list  #MAKE SURE TO RETURN THE LIST AND THEN YOU CAN ENTER THIS LIST INTO ANOTHER FUNCTION AND FURTHER FILTER.
